...
try {
    ...
    console.warn('Dimensions not available during BakeryScreen initialization');
}
    ...
    const loadData = useCallback(async (showLoading = true, options = {}) => {
        ...
        try {
            const [items, tax] = await Promise.all([
                getBakeryItems(options),
                getTaxByService('bakery', options),
            ]);
