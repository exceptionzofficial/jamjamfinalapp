...
const CATEGORIES = [
    ...
    { id: 'non-veg', name: 'Non-Veg', icon: 'food-drumstick' },
...
// Loading animation
const RestaurantLoadingAnimation = require('../assets/food.json');
...
const RestaurantScreen = ({ route, navigation }) => {
    ...
    const loadData = useCallback(async (showLoading = true, options = {}) => {
        ...
        try {
            ...
            setMenuItems(restaurantOnlyItems);
            setCombos(comboList);
        ...
        return Object.entries(cart)
            ...
            .map(([itemId, quantity]) => {
                ...
                return {
                    ...item,
                    quantity,
                    subtotal: (item.price || item.comboPrice) * quantity,
                };
            })
            .filter(Boolean);
    }, [cart, menuItems, combos]);
    ...
    const handleSaveMenuItem = async () => {
        ...
        try {
            ...
            if (editingItem) {
                await updateMenuItem(editingItem.itemId || editingItem.id, itemData);
        ...
        Alert.alert(
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            await deleteMenuItem(item.itemId || item.id);
                            loadData(false);
    ...
    }, [isEditMode, isDeleteMode, openEditMenu, handleDeleteMenuItem, addToCart]);
                        ...
                        <>
                            ...
                            <View style={[styles.cartFooter, { borderTopColor: colors.border }]}>
                                <View style={styles.cartTotal}>
                                    ...
                                    <Text style={[styles.cartTotalValue, { color: colors.textPrimary }]}>₹{taxInfo.subtotal}</Text>
                                </View>
                                ...
                                <View style={[styles.cartTotal, { borderTopWidth: 1, borderTopColor: colors.border, paddingTop: 8, marginTop: 8 }]}>
                                    ...
                                    <Text style={[styles.cartTotalValue, { color: colors.brand }]}>₹{taxInfo.total}</Text>
                                </View>
                                <TouchableOpacity
                ...
                <View style={[styles.modalContainer, { backgroundColor: colors.background }]}>
                    ...
                    >
                        <View style={styles.paymentAmountSection}>
                            ...
                            {taxInfo.taxPercent > 0 && (
                                <Text style={[styles.paymentTaxInfo, { color: colors.textSecondary }]}>
    ...
    if (isLoading) {
        return (
            <View style={[styles.container, styles.loadingContainer, { backgroundColor: colors.background }]}>
                <LottieView
                    source={RestaurantLoadingAnimation}
                    autoPlay
                    loop
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            <Header subtitle="Restaurant" showTypewriter={false} />
